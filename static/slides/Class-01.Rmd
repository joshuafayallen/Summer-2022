...
- Number of Times I have taught this class: 2 

- Number of Times I have taught this class in person: 1]
...
- Walk up song: Simon Says: Pharoahe Monach
...
- The slides live on this website [https://spols110122.netlify.app/](https://spols110122.netlify.app/)
...
- If you are curious about how they are made just visit [`r icons::fontawesome("github")`](https://github.com/joshuafayallen/Summer-2022-Website/tree/main/static/slides)
...
exam_dates <- c(ymd(20220622), ymd(20220713), ymd(20220725))


activities = c(ymd(20220619, 20220710, 20220717))

# What are the full dates of the semester? Here, I'll exclude exam week as I like to do.
# In this case: 6 January to 23 April
semester_dates <- seq(ymd(20220606), ymd(20220727), by=1)
...
tibble(date = seq(ymd(20220606), ymd(20220727), by=1))  %>%
  mutate(mon = lubridate::month(date, label=T, abbr=F), # get month label
         ...
         activity = ifelse(date %in% activities, 1,0),# is it a day off?
...
Cal %>%
  mutate(category = NA,
         ...
         category = ifelse(semester == 1 & wkdy %in% c("Mon", "Wed"), "Class Day", category),
         category = ifelse(exams == 1, "Exams", category),
         category = ifelse(activity == 1, "Activity", category),
...
Cal %>% 
  ...
  scale_fill_manual(values=c( "Activity" = "cadetblue",
                              "Class Day"="steelblue", 
                             "Semester"="lightsteelblue",
                              "NA" = "white", # I like these whited out...
                             "Exams"="indianred4"),
                    #... but also suppress a label for a non-class semester day
                    breaks=c("Activity", "Class Day","Exams", "Not Here")) +
  labs(fill = "", x="", y="", caption = "We do not have class on June 20 or July 4")  -> class_cal
...
???
These next two slides are basically like the tl;dr of the syllabus. This calendar is just taken directly from the syllabus. The third exam is going to be a tight turnaround. My grades are due on thursday of that week. So please talk to me in advance if you will miss it.  
---
...
grades_df = tribble(~Assignment, ~Weight,
                    ...
                    "Second Highest Exam Score", "16%",
                     "Lowest Exam", "12%",
                    "Chapter Readings and probes in Area9", "35%",
                    "Activities", "15%")
...
???
So if you look at the breakdown of the class half of your grade is going to be determined by exam grades and the other half on the stuff you do outside of class. It is going to be critical that you do all the stuff. Unfortunately we are kind of constrained by the fact that this is shortened class so lots of the things that offset exam grades just simply are not available for this class. 
