...
???
This is pretty easy! it is just 2/5 or 40% and this should be true as we pick candies. So lets test this out with some simulations! This definition is not technically correct but it is useful for our purposes. 
...
???
We not only care about the likelihood that something happens but we also care about the uncertainty around the event. This is something our simple definition of probability does not handle well. Lets say we randomly assign an increase in medicare coverage(something that acutally happened). We are interested in the likelihood that we see an update in use of insurance, how many people increase the use, but we also care about the other outcomes of the expansion and the uncertainty around those numbers 
...
- This affects our confidence intervals because this is the forumula $CI = \bar{x}\pm z \frac{\sigma}{\sqrt{n}}$
