...
monty_hall = function(strategy){
  doors = as.character(1:3) ## make the doors
  prize = sample(c("car", "goat", "goat")) ## set the choices
  prize_door = doors[prize == "car"] ## set the prize
  my_pick  = sample(doors, 1)  ## make the pick 
  show = sample(doors[!doors %in% c(my_pick, prize_door)],1) ## show a door with a goat behind it 
  stick = my_pick 
  stick == prize_door 
...
```{r echo = TRUE}
stick = replicate(B, monty_hall("stick"))  ### run the simulation if we stick with our choice 
mean(stick)

switch = replicate(B, monty_hall("switch")) ## run the simulation if we change our pick
...
normal_dist("#e76254", "#1e466e", "Open Sans") + 
  theme_allen()  +
    labs(title = "The Area Underneath a Normal Distribution",
       y = "Density",
       x = "") + scale_x_continuous(breaks = c(-4, -2.58, -1.96, -1.64, -1, 0, 1, 1.64, 2.58, 4)) + theme(axis.text.x = element_text(size = rel(0.65),  margin = ggplot2::margin(t = 10)))
...
so lets see how sample sizes effect tings again.
---
